...
class showSpider(scrapy.Spider):
    ...
    # 每个爬虫指定管道
    custom_settings = {
        'ITEM_PIPELINES': {'xiudong.pipelines.showPipeline': 500}
    }
    ...
    def show_parse(self, response):
        ...
        if len(img_sellist) > 0:
            item["show_img"] = img_sellist.extract_first()
        # 演出名称
        if len(name_sellist) > 0:
            item["show_name"] = name_sellist.extract()[-1].strip()
        # 演出时间
        item["show_time"] = list_sellist[0].xpath('./text()').extract_first().replace(" ","").replace("\t","").replace("\r","").replace("\n","")
        # 艺人
        item["show_artists"] = []
        artists_sellist = list_sellist[1].xpath('./a/text()')
        for artist_item in artists_sellist:
            item["show_artists"].append(artist_item.extract())
        # 场地
        item["show_place"] = list_sellist[2].xpath('./a/text()').extract_first()
        # 地址
        item["show_addr"] = list_sellist[3].xpath('./text()').extract_first().replace(" ","").replace("\t","").replace("\r","").replace("\n","")
        # 电话
        item["show_phone"] = list_sellist[4].xpath('./text()').extract_first().replace("电话：","").replace(" ","").replace("\t","").replace("\r","").replace("\n","")
        # 标签
        item["show_tag_list"] = []
        for tag_item in tag_sellist:
            item["show_tag_list"].append(tag_item.extract())
        # 价格 -- status -- 1：没有库存，2：未开始售票，3：售票结束，4：售票中，5：活动取消，6：审核中
        item["show_price_list"] = []
        for price_item in price_sellist:
            ...
            item["show_price_list"].append(price_item_dict)
        # 演出详情
        item["show_details"] = show_details_sellist.extract_first()
        # 主办方
        if len(sponsor_sellist) > 0:
            item["show_sponsor"] = sponsor_sellist.extract_first()
        else:
            item["show_sponsor"] = ""
        # 相关图片
        item["show_about_imgs_list"] = []
        for img_item in imgs_sellist:
            item["show_about_imgs_list"].append(img_item.extract())
        # 音乐作品
        item["show_music_dict"] = {}
        for music_item in music_sellist:
            item["show_music_dict"]["music_name"] = music_item.xpath('./span[@class="a-link"]/text()').extract_first()
            item["show_music_dict"]["music_artist"] = music_item.xpath('./span[@class="singing"]/a/text()').extract_first()
            item["show_music_dict"]["music_url"] = music_item.xpath('./div[@class="jp-jplayer"]/@playsrc').extract_first()
        yield item
